...
export default function ScannerPage() {
  ...
  const submitLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (r.ok) { setAuthed(true); setPass(""); }
      else {
        const d = await r.json().catch(() => ({}));
        setLoginErr(d.error || "Code invalide");
      }
    } catch {
  ...
  if (authed === null) {
    return (
      <div style={{ minHeight: "100dvh", background: "linear-gradient(135deg,#0A1A4A,#0F2560,#1C0F06)", display: "flex", alignItems: "center", justifyContent: "center", color: "#F5ECD7", fontFamily: "Arial, Helvetica, sans-serif" }}>
  ...
  if (!authed) {
    return (
      <div style={{ minHeight: "100dvh", background: "linear-gradient(135deg,#0A1A4A,#0F2560,#1C0F06)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, color: "#F5ECD7", fontFamily: "Arial, Helvetica, sans-serif" }}>
        <div style={{ textAlign: "center", marginBottom: 22 }}>
          <div style={{ fontFamily: "Georgia,serif", fontSize: 24, letterSpacing: 5, color: "#fff", fontWeight: 700 }}>ESEN</div>
          <div style={{ color: "#F0B429", fontSize: 10, letterSpacing: 3, marginTop: 4 }}>GRADUATION 2026</div>
          <h1 style={{ fontFamily: "'Playfair Display', Georgia, serif", color: "#fff", fontSize: 20, marginTop: 14 }}>Scanner d&apos;entrée</h1>
          <div style={{ width: 56, height: 2, background: "linear-gradient(90deg,#1B3A8C,#F0B429,#1B3A8C)", margin: "10px auto 0" }} />
        </div>
        <form onSubmit={submitLogin} style={{ background: "rgba(15,37,96,0.4)", border: "1px solid rgba(240,180,41,0.35)", borderRadius: 16, padding: 22, width: "100%", maxWidth: 340, backdropFilter: "blur(6px)" }}>
          <div style={{ fontSize: 13, color: "#F5ECD7", marginBottom: 10, textAlign: "center" }}>Code d&apos;accès staff</div>
          <input
            type="password"
            value={pass}
            ...
            style={{ width: "100%", padding: "12px 14px", borderRadius: 10, background: "rgba(10,15,30,0.7)", border: "1px solid rgba(240,180,41,0.35)", color: "#fff", fontSize: 15, boxSizing: "border-box", textAlign: "center", letterSpacing: 3 }}
          />
          {loginErr && <div style={{ marginTop: 10, color: "#fca5a5", fontSize: 12, textAlign: "center" }}>{loginErr}</div>}
          <button type="submit" disabled={loginBusy || !pass} style={{ marginTop: 14, width: "100%", padding: "12px 18px", borderRadius: 10, background: "linear-gradient(135deg,#F0B429,#e08c1a)", border: "none", color: "#0A1A4A", fontSize: 14, fontWeight: 800, letterSpacing: 1, cursor: "pointer", opacity: loginBusy ? 0.6 : 1 }}>
            {loginBusy ? "…" : "Ouvrir le scanner"}
          </button>
        </form>
        <div style={{ marginTop: 18, fontSize: 11, color: "rgba(245,236,215,0.5)", textAlign: "center", maxWidth: 320 }}>
          Accès réservé au staff. Session valide 24 h sur cet appareil.
        </div>
      </div>
    );
  }
