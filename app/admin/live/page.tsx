...
export default function LiveDashboardPage() {
  ...
  const full = !!stats && stats.totalExpected > 0 && inside >= stats.totalExpected;
  ...
  return (
    <div style={shell}>
      ...
      {banner && (
        <div style={{
          position: "fixed", top: "12%", left: "50%", transform: "translateX(-50%)", zIndex: 100000,
          background: "linear-gradient(135deg,#F0B429,#FFD166)", color: "#0A1A4A",
          padding: "16px 34px", borderRadius: 16, fontSize: "clamp(18px,3vw,34px)", fontWeight: 800,
          boxShadow: "0 20px 60px rgba(0,0,0,0.5)", animation: "celebPop 0.5s ease", whiteSpace: "nowrap",
        }}>{banner}</div>
      )}
      {/* Header */}
      ...
      <div style={{
        ...
        <div style={{ fontFamily: "Georgia, serif", fontSize: "clamp(72px,16vw,220px)", fontWeight: 800, color: "#fff", lineHeight: 1, animation: "breathe 3.6s ease-in-out infinite", transformOrigin: "center" }}>
          {insideA}
        </div>
      ...
      <div style={{ marginBottom: "clamp(16px, 2vw, 32px)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
          ...
          <span style={{ fontFamily: "Georgia, serif", fontSize: "clamp(28px,4vw,52px)", fontWeight: 800, color: "#34D399" }}>{rateA}%</span>
        </div>
...
function LiveCard({ title, checked, total, accent }: { title: string; checked: number; total: number; accent: string }) {
  ...
  const checkedA = useCountUp(checked);
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
        <span style={{ fontFamily: "Georgia, serif", fontSize: "clamp(48px,8vw,110px)", fontWeight: 800, color: "#fff", lineHeight: 1 }}>{checkedA}</span>
